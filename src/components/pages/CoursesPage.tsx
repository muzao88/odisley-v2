...
import { CONTEUDOS_SEED, CATEGORIAS_ORDER, CATEGORIA_CORES, CATEGORIA_ICONES } from '@/data/conteudos';
...
export default function CoursesPage({ onNavigate, onSelectConteudo }: Props) {
    ...
    const fetchConteudos = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setConteudos(CONTEUDOS_SEED.map(c => ({ ...c, _id: c.nome, aulasConcluidasCount: 0, percentual: 0 })) as any);
        }
      } catch {
        setConteudos(CONTEUDOS_SEED.map(c => ({ ...c, _id: c.nome, aulasConcluidasCount: 0, percentual: 0 })) as any);
      } finally {
  ...
  const byCategoria = CATEGORIAS_ORDER.reduce<Record<Categoria, ConteudoComProgresso[]>>((acc, cat) => {
  ...
  if (loading) {
    return (
      <div className="page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
        <div style={{ color: 'var(--text3)', fontFamily: "'Syne',sans-serif" }}>Carregando conteúdos...</div>
      </div>
    );
  }
    ...
    <div className="page">
      <section>
        ...
        <p className="section-sub" style={{ marginBottom: '3rem' }}>
          24 conteúdos organizados do básico ao avançado. Clique em qualquer card para começar.
        </p>
          ...
          return (
            <div className="category-section" key={cat}>
              <div className="category-header">
                <div className="category-icon" style={{ background: `${cor}20` }}>
              ...
              <div className="conteudo-grid">
                {items.map(c => (
                  <div key={c._id} className="conteudo-card"
                    ...
                    <div className="cc-bar-wrap">
                      <div className="cc-bar-label">
                        <span>{c.aulasConcluidasCount}/{c.totalAulas} concluídas</span>
                        <span style={{ color: cor, fontWeight: 600 }}>{c.percentual}%</span>
                      </div>
                      <div className="cc-bar">
                        <div className="cc-bar-fill" style={{ width: `${c.percentual}%`, background: cor }} />
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* Upsell */}
        <div style={{
          marginTop: '2rem', padding: '1.75rem 2rem',
          background: 'var(--surface)', border: '1px solid var(--border)',
          borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center',
          justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem',
        }}>
          <div>
            <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, marginBottom: '.3rem' }}>🔒 Aulas bloqueadas?</div>
            <div style={{ color: 'var(--text2)', fontSize: '.88rem' }}>
              Assine o plano premium para desbloquear todos os conteúdos e simulados.
            </div>
          </div>
          <button className="btn btn-primary btn-md" onClick={() => onNavigate('planos')}>
            Ver planos →
          </button>
        </div>
      </section>
